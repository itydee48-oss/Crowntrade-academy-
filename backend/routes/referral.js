...
router.get('/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    if (!application) return res.status(404).json({ error: 'No partner application found' });
    ...
    const showWelcome = application.status==='approved' && application.welcomed===false;
    if (showWelcome) await query('UPDATE referral_applications SET welcomed=TRUE WHERE id=$1', [application.id]);
    ...
    res.json({
      agent: { id:user.id,member_number:user.member_number,full_name:user.full_name,email:user.email,phone:user.phone,
        ...
        status:application.status,tier:currentTier.name,tier_label:currentTier.label,tier_rate:currentTier.rate,tier_color:currentTier.color,
