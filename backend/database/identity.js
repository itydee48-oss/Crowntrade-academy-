...
async function findOrCreateUser({ full_name, email, phone, password }) {
  ...
  const result = await query(`INSERT INTO users (member_number,full_name,email,phone,password_hash,status) VALUES ($1,$2,$3,$4,$5,'pending') RETURNING *`,
...
async function grantMentorship(userId) {
  await query(`UPDATE users SET has_mentorship=TRUE,has_partner_status=TRUE,status='active',updated_at=CURRENT_TIMESTAMP WHERE id=$1`, [userId]);
}

async function grantPartnerStatus(userId, { tier = 'bronze', referralCode } = {}) {
  await query(`UPDATE users SET has_partner_status=TRUE,partner_tier=$2,referral_code=COALESCE(referral_code,$3),status='active',updated_at=CURRENT_TIMESTAMP WHERE id=$1`,
